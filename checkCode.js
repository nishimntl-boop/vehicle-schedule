    ...
    current.events.forEach(e => {
      ...
      if (incomingById.has(id)) {
        ...
        if (hasDirtyList && !dirty.has(id)) {
          ...
          // dirtyとして明示された編集は、同一端末で直前に行った操作を優先。
          // ただし、明らかにサーバー側の方が新しい場合だけ競合として維持する。
          const serverNow = now;
          const incomingRecent = incomingTs > 0 && (serverNow - incomingTs) < 24 * 60 * 60 * 1000;
          const currentRecent = currentTs > 0 && (serverNow - currentTs) < 24 * 60 * 60 * 1000;
          if (dirty.has(id) && incomingRecent && (!currentRecent || incomingTs >= currentTs)) mergedEvents.push(incoming);
          else mergedEvents.push(incomingTs >= currentTs ? incoming : e);
        }
      } else {
        mergedEvents.push(e);
      }
