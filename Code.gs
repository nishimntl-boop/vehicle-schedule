...
function doGet(e){try{if(e&&e.parameter&&e.parameter.api==='1'){const forceFresh=String(e.parameter.nocache||'')==='1';const state=readState_(forceFresh);const cb=e.parameter.prefix||e.parameter.callback;return cb?jsonp_(cb,state):json_(state);}if(e&&e.parameter&&e.parameter.health==='1')return json_({ok:true,spreadsheet:'connected'});return HtmlService.createHtmlOutputFromFile('index').setTitle('車両人員予定表').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);}catch(err){return HtmlService.createHtmlOutput('<pre>起動エラー: '+String(err&&err.message||err).replace(/</g,'&lt;')+'</pre>');}}
