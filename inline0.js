...
function MONTH(){
  ...
  for(let i=0;i<42;i++){const d=new Date(s);d.setDate(s.getDate()+i);const cn='day '+(d.getMonth()!=m?'out ':'')+(K(d)===K(cur)?'selected':'');const dn=d.getDay()===0?'sun':d.getDay()===6?'sat':'';const dayEvents=events.filter(e=>ON(e,d));h+=`<div class="${cn}" onclick="selectDay('${K(d)}')"><div class="date ${dn}">${d.getDate()}</div><div class="day-events">${dayEvents.map(e=>BAR(e,d)).join('')}</div></div>`;}return h+'</div>';
}
function WEEK(){
...
function DAY(){
  ...
  const labels=Array.from({length:25},(_,i)=>i===24?'24:00':String(i).padStart(2,'0')+':00');
  let h='<div class="timeline"><div class="times"><div>車両 / 人員</div><div class="time-scale">'+labels.map((x,i)=>`<div class="time-label" style="left:${(i/24)*100}%">${x}</div>`).join('')+'</div></div>';
  events.filter(e=>ON(e,cur)).forEach(e=>{const seg=SEG(e,cur);if(!seg)return;const[a,b]=seg;const dayStart=new Date(K(cur)+'T00:00');const nextDay=new Date(dayStart);nextDay.setDate(nextDay.getDate()+1);const sh=Math.max(0,Math.min(24,(a-dayStart)/3600000));const eh=Math.max(0,Math.min(24,(b-dayStart)/3600000));const l=(sh/24)*100,r=(eh/24)*100;if(r<=0||l>=100||r<=l)return;const w=Math.max(1,r-l),night=nightOnDay(e,cur);h+=`<div class="trow"><div class="who">${E((e.vehicles||[]).join(', '))}<br>${E((e.people||[]).join(', '))}</div><div class="track"><button class="tbar ${e.kind==='off'?'off':e.kind==='plan'?'plan':''} ${CF(e)?'conflict':''} ${night?'overnight':''}" style="left:${l}%;width:${w}%;background:${BG(e,cur)}" onclick="editEvent('${E(e.id)}','${K(cur)}')">${E(e.kind==='off'?'休み':(night?'🌙 ':'')+(e.kind==='plan'?'予定 ':'')+(e.site||'未設定')+' '+e.start+'～'+e.end)}</button></div></div>`;});
  return h+'</div>';
}
let appZoom=Number(localStorage.getItem('vehicleAppZoom')||'1');
function applyZoom(){
  appZoom=Math.max(0.6,Math.min(1.6,Math.round(appZoom*10)/10));
  const z=document.getElementById('zoomContent');
  if(z){
    z.style.transformOrigin='top left';
    z.style.transform=`scale(${appZoom})`;
    z.style.width=`${100/appZoom}%`;
    requestAnimationFrame(()=>{
      const h=z.scrollHeight;
      z.style.minHeight='0';
      z.style.marginBottom=`${Math.max(0,h*(appZoom-1))}px`;
    });
  }
  const v=document.getElementById('zoomValue');
  if(v)v.textContent=Math.round(appZoom*100)+'%';
  localStorage.setItem('vehicleAppZoom',String(appZoom));
}
function changeZoom(delta){appZoom=Math.round((appZoom+delta)*10)/10;applyZoom();}
function resetZoom(){appZoom=1;applyZoom();}

/* v47fix18: 週予定だけの2本指ピンチ縮小/拡大。固定ヘッダー・月予定・日予定には影響しない。 */
(function(){
  let pinch={el:null,startDist:0,startScale:1};
  function dist(t){const a=t[0],b=t[1];return Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY);}
  function weekAt(target){return target&&target.closest?target.closest('.week'):null;}
  document.addEventListener('touchstart',e=>{if(e.touches.length!==2)return;const w=weekAt(e.target);if(!w)return;pinch={el:w,startDist:dist(e.touches),startScale:Number(getComputedStyle(w).getPropertyValue('--week-scale'))||1};},{passive:true});
  document.addEventListener('touchmove',e=>{if(!pinch.el||e.touches.length!==2)return;const d=dist(e.touches);if(!d)return;const scale=Math.max(.55,Math.min(1.25,pinch.startScale*d/pinch.startDist));pinch.el.style.setProperty('--week-scale',String(Math.round(scale*100)/100));e.preventDefault();},{passive:false});
  document.addEventListener('touchend',e=>{if(e.touches.length<2)pinch.el=null;},{passive:true});
})();
...
function newEvent(){eid=null;assignmentTouched={desktop:false,mobile:false};editAssignmentSnapshot=null;const blank={kind:'plan',site:'',work:'',date:K(cur),endDate:K(cur),start:'08:00',end:'16:00',vehicles:[],people:[]};setFormValues('m',blank);document.getElementById('ftitle').textContent='予定の追加';document.getElementById('mftitle').textContent='予定の追加';openEventModal();}
function editEvent(id,d){assignmentTouched={desktop:false,mobile:false};const e=events.find(x=>String(x.id)===String(id));if(!e)return;cur=new Date(d+'T00:00');eid=e.id;editAssignmentSnapshot={vehicles:(e.vehicles||[]).map(String),people:(e.people||[]).map(String)};setFormValues('m',e);document.getElementById('ftitle').textContent='予定の編集';document.getElementById('mftitle').textContent='予定の編集';render();openEventModal();}
...
rebuildChecks();render();applyZoom();loadCloud();
